...
@Component
public class OnboardingStatusChangedListener {
    private final IntegrationEventJsonReader eventReader;
    private final JourneyResolutionProjector projector;
    
    public OnboardingStatusChangedListener(
        IntegrationEventJsonReader eventReader, 
        JourneyResolutionProjector projector
    ) {
        this.eventReader = eventReader;
        this.projector = projector;
    }
    
    @KafkaListener(topics = "${journey.kafka.topics.onboarding}")
    public void consume(String payload){
        var event = eventReader.read(payload,OnboardingStatusChangedEvent.class);
